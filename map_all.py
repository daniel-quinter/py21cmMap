...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="A command-like script to map a cube onto a set of shells in comoving space."
    )
   ...
   # data_cube = np.random.normal(scale = np.sqrt(1), size=(100, 100, 100))    
    shells = [cosm_plot(redshift, nside, cube_len, data_cube) for redshift in redshifts]
